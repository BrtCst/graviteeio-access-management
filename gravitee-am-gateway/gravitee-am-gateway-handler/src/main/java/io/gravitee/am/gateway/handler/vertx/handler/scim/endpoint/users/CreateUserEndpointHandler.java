...
public class CreateUserEndpointHandler implements Handler<RoutingContext> {
        ...
        userService.create(user)
                .subscribe(
                        user1 -> context.response()
                                ...
                                .putHeader(HttpHeaders.LOCATION, location(context.request(), user1))
                                .end(Json.encodePrettily(user1)),
    ...
    private String location(HttpServerRequest request, User user) throws URISyntaxException {
        return UriBuilderRequest.resolveProxyRequest(request, request.path() + '/' + user.getId(), null);
    }
}
